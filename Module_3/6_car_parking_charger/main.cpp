...
struct ParkingException {
    ParkingException() = default;
    ParkingException(const std::string& message) : e(message){}
    ~ParkingException() {}
    std::string e;
...
public:
    ...
    void Park(VehiclePlate car) {
        if (now_parked_.find(car) != now_parked_.end()) {
            throw ParkingException("Already parked"s);
        } else {
    ...
    void Withdraw(const VehiclePlate& car) {
        if (now_parked_.find(car) == now_parked_.end()) {
            throw ParkingException("Not parked yet"s);
        }
        Duration duration_ = Clock::now() - now_parked_[car];
        complete_parks_[car] += duration_;
    ...
    int64_t GetCurrentBill(const VehiclePlate& car) const {
        ...
        if (now_parked_.find(car) != now_parked_.end()) {
            now_ = std::chrono::duration_cast<std::chrono::seconds>(Clock::now() - now_parked_.at(car)).count();
        }
        if (complete_parks_.find(car) != complete_parks_.end()) {
            then_ = std::chrono::duration_cast<std::chrono::seconds>(complete_parks_.at(car)).count();
        } 
        int cost_ = (now_ + then_) * cost_per_second_;
        return cost_;
    }
    ...
    unordered_map<VehiclePlate, int64_t, VehiclePlateHasher> EndPeriodAndGetBills() {
        ...
        for (const auto& [car_, time_point_] : now_parked_) {
            int dur_ = std::chrono::duration_cast<std::chrono::seconds>(Clock::now() - time_point_).count();
            if (dur_ > 0) {
                result[car_] = dur_ * cost_per_second_;
                now_parked_[car_] = Clock::now();
            } 
        }

        for (const auto& [car_, dur_] : complete_parks_) {
            int duration_ = std::chrono::duration_cast<std::chrono::seconds>(dur_).count();
            if (duration_ > 0) {
                result[car_] += duration_ * cost_per_second_;
                complete_parks_[car_] = std::chrono::seconds(0);
            }
        }
...
int main() {
    ...
    std::cout << parking.GetCurrentBill({'B', 'B', 222, 'B', 99});
