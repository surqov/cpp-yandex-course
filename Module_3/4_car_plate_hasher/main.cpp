...
public:
    void Insert(T elem) {
        ...
        bool is_duplicate = elements_.size() > index && 
            elements_.at(index).size() > 0 && 
            std::none_of(elements_.at(index).begin(), elements_.at(index).end(), [elem](const auto& elem_){
                return !(elem == elem_);
            });
    ...
    void PrintAll(ostream& out) const {
        for (const vector<T>& plates : elements_) {
