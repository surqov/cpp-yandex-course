...
void MergeSort(RandomIt range_begin, RandomIt range_end) {
    ...
    auto begin_ = elements.begin();
    auto end_ = elements.end();
    if (range_length < 1500000) {
        MergeSort(begin_, mid);
        MergeSort(mid, end_);
    } else {
        auto left_ = async ( [begin_, mid]{ MergeSort(begin_, mid); } );
        auto right_ = async ( [end_, mid]{ MergeSort(mid, end_);} );
    }
